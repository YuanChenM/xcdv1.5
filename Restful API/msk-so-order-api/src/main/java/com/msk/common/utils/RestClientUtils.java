...
public final class RestClientUtils {
     ...
     *
     */
    public static <T extends Serializable> RestResponse<T> post(String url, Object param, TypeReference<RestResponse<T>> typeReference, SerializerFeature ... serializerFeature){
     ...
     * @param param 请求参数
     */
    public static <T> void asyncPost(final String url,final Object param,final AsyncPostCallBack<T> asyncPostCallBack){
        final TypeReference<T> tTypeReference = new TypeReference<T>(){};
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                String jsonParam = JSON.toJSONString(param);
                logger.debug("请求URL:"+url+"请求参数:"+jsonParam.replace("\n","").replace("\r","").replace(" ",""));
                String resultJson = post(url,jsonParam);

                T entity = JSON.parseObject(resultJson, tTypeReference);
                asyncPostCallBack.callBack(entity);
            }
        };
        Thread thread = new Thread(runnable);
        thread.start();
    }
    public static <T> void asyncPost(final String url,final Object param,final AsyncPostCallBack<T> asyncPostCallBack,final TypeReference<T> tTypeReference){

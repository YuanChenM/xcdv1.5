...
import com.msk.print.bean.ResponseMessage;
...
public class HtmlUtil {
	...
	private static org.slf4j.Logger Logger = LoggerFactory.getLogger(HtmlUtil.class);
	...
	public synchronized static void writerFailJson(HttpServletResponse response, String errorMessage) {
		...
		try {
			...
			response.setContentType("application/json");
			writer(response, JSON.toJSONString(errorMessage));
		} catch (Exception e) {
			Logger.error(e.getMessage(), e);
		}
	}

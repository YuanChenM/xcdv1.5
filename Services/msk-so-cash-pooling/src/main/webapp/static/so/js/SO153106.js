var SO153106 = {
    formId: "so153106Param",
    saveButtonId: "SO153106_SAVE",
    backButtonId: "SO153106_BACK",
    operateTimeT: "#operateTime",
    init: function () {
        this.bindSavebutton();
        this.bindBackbutton();
        this.bindDatePicber(SO153106.operateTimeT),
            this.changeSelect();
    },
    ...
    bindSavebutton: function () {
        $("#" + SO153106.saveButtonId).click(function () {
            SO153106.uploadData();
        });
    },
    bindBackbutton: function () {
        $("#" + SO153106.backButtonId).click(function () {
            $.pdialog.close();
        });
    },
    uploadData: function () {
        $.alertMessage.confirm("你确定要保存当前数据吗？", function () {
            $.alertMessage.close();
            var formData = getFormData($("#" + SO153106.formId));
            var count = 0;
            // Modify for Bug#1652 by li_huiqian at 2016/9/8 start
            // 必须check
            $("#SO153106DataGrid>tbody>tr").each(function () {
                // 重置输出框颜色
            ...
            //}
            //// 退款编码 验证
            //var refundCode = formData.refundCode;
            //if (amountType == '1' && refundCode == '') {
            //    count++;
            //    $("#refundCode").css('background', 'yellow');
            //} else {
            //    $("#refundCode").css('background', 'none');
            //}
            //// 支付流水号 验证
            ...
            if (count > 0) {
                ...
                var fromUrl = $("#" + SO153106.formId).attr("action");

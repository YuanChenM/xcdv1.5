...
var HDF = {
	...
	jsonEval : function(data) {
		try {
			...
			try{
				if($(data).html() == 'timeout'){
					return { statusCode : HDF.statusCode.timeout };
				}else{
					return {};
				}
			}catch(e){
...
(function($) {
	...
	$.fn.extend({
		...
		ajaxUrl : function(op, settings) {
			...
			$.ajax($.extend({
				type : op.type || 'GET',
				// Modify for Bug#0201751 at 20150609 by liu_jinliang start
				// IE9未对URI做处理，导致含中文的请求乱码
				url : encodeURI(op.url),
				// Modify for Bug#0201751 at 20150609 by liu_jinliang end
				data : op.data,
				cache : false,
				success : function(response) {
					try {
						...
						if (json.statusCode == HDF.statusCode.timeout) {
							$("html").html(response);
						}else if (json.statusCode == HDF.statusCode.error) {
							if (json.message) {

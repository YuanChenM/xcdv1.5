 ...
 */
@Controller
@RequestMapping("BR121408")
public class BR121408Controller extends BaseController {

    private static Logger logger = LoggerFactory.getLogger(BR121408Controller.class);
    ...
    public String init(Model model) {
        logger.debug("分销买家池营销期公众买家分池买家注册管控初始化");
        //获取文件下载路径
    ...
    List<BrMPdClasses> findMachiningList(@PathVariable("classesCode") String classesCode) {
        ...
        if (!StringUtil.isEmpty(classesCode)) {
            ...
            request.setParam(param);
            String pdUrl = SystemServerManager.BuyersReportServerManager.getFindMachiningCodeU();
//            String pdUrl = "http://localhost:8380/msk-br-api/api/br/brMPdClasses/findMachiningCodeU";
    ...
    PageResult<BR121405Bean> search(BasePageParam param) {
        param.setFilter("poolType", StringUtil.toString(NumberConst.IntDef.INT_FOUR));
    ...
    public
    @ResponseBody
    IBR121408RsBean generateBuyerPoolFileInfo(IBR121408RsParam param) {
        RsRequest<IBR121408RsParam> request = new RsRequest<>();
        request.setAuth("MSK00001");
        request.setLoginId("msk01");
        request.setSiteCode("1");
        request.setParam(param);
        String url = SystemServerManager.BuyersReportServerManager.getCreateMarketingPeriodExcel();
//        String url = "http://localhost:8180/api/br/createMarketingPeriodExcel/generateAndUpload";
        RsResponse<IBR121408RsBean> list = RestClientUtil.post(url, request, new TypeReference<RsResponse<IBR121408RsBean>>() {
        });
        ...
        if(list != null &&  list.getResult() != null ){
            ...
            ibr121408RsBean.setDataCount("0");
            return ibr121408RsBean;
        }
    }
    ...
    public @ResponseBody
    void  createExcel(IBR121408RsParam param,Model model){
        RsRequest<IBR121408RsParam> request = new RsRequest<>();
        request.setAuth("MSK00001");
        request.setLoginId("msk01");
        request.setSiteCode("1");
...
//        String url = "http://localhost:8180/api/br/createMarketingPeriodExcel/generateAndUpload";
    ...
    public
    @ResponseBody
    IBR121408RsParam deleteExcel(@PathVariable("fileId") String fileId) {
        logger.info("删除营销期公众买家文件信息");
        RsRequest<IBR121408RsParam> paramRsRequest = new RsRequest<>();
        ...
        String url = SystemServerManager.BuyersReportServerManager.getCreateMarketingPeriodExcel();
//        String url = "http://localhost:8180/api/br/createMarketingPeriodExcel/generateAndUpload";
        RsResponse<IBR121408RsParam> response = RestClientUtil.post(url, paramRsRequest, new TypeReference<RsResponse<IBR121408RsParam>>() {
        });
        if(response != null &&  response.getResult() != null ){
            ...
            ibr121408RsParam.setDataCount("0");
            return ibr121408RsParam;
        }
    }
}

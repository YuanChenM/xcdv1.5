...
public class BaseRedisDao {
    ...
    /**
     * 获得Redis Map 中指定field的Value
     * @param key key
     * @field field field
     * @return value
     */
    public String getRedisMapFieldValue(final String key,final String field){
        String result = (String) this.redisTemplate.execute(new RedisCallback() {
            public String doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                byte[] byteValue= connection.hGet(byteKey, byteField);
                String value = serializer.deserialize(byteValue);
                return value;
            }
        });
     ...
     * @author shi_yuxi
     */
    public void delRedisMapOne(final String key, final String field) {
         this.redisTemplate.execute(new RedisCallback() {
            public Long doInRedis(RedisConnection connection) throws DataAccessException {
                RedisSerializer serializer = getRedisSerializer();
                byte[] byteKey = serializer.serialize(key);
                byte[] byteField = serializer.serialize(field);
                return connection.hDel(byteKey, byteField);
            }
        });

...
<navigation:header title="价格申报详细" backTitleArray="产品价盘录入（简易版）"  backUrlArray="${ctx}/SP171116/init/"></navigation:header>

<div class="page-content list-page">
    <form action="${ctx}/SP171117/save" id="SP171117Form" method="post">
        ...
        <input type="hidden" id="overDataFlag" name="filterMap['overDataFlag']" value="${overDataFlag}">
    <div class="group-accordion" collapsible="false" active="false">
        <h3>
            <label>采购员价格申报详细</label>
        </h3>
    ...
    <div class="group-accordion" collapsible="false" active="false">
        <h3>
            ...
            <table class="dataTable no-footer" id="SP171117Table" style="width: 100%">
                <thead>
                <tr>
                    ...
                    <tr>
                        ...
                        <c:if test="${priceDetailInfo.wayGradeCode eq defaultWayGrade}">
                            <td align="center">
                                ...
                                <input type="text" id="price${status.index}" name="wayGradePriceArray" value="${priceDetailInfo.wayGradePrice}" <c:if test="${overDataFlag eq '1'}">disabled</c:if>>
                            </td>
                        </c:if>
                        ...
                        <input type="hidden" name="marketingNameArray" value="${priceDetailInfo.marketingName}">
                        ...
                        <td style="display:none;">
                            <input type="hidden" id="tempPrice${status.index}" min="0" name="wayGradeTempPriceArray" value="${priceDetailInfo.wayGradeTempPrice}">
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
    ...
    <br>
     <span>
    <c:if test="${overDataFlag ne '1'}">
            <msk:button buttonValue="生成价盘" buttonId="SP171117.SAVE"  buttonType="button" />
    </c:if>
         <%-- Modfiy:  一览页面到详情页面打开方式横展开,将返回按钮注释   2016/10/11   BY  zhukai1  Start --%>
   <%-- <msk:button buttonValue="返回" buttonId="SP171117.RETURN" url="${ctx}/SP171116/init?pricePeriod=${sp171110Param.pricePeriod}" buttonType="button"/>--%>
          <%-- Modfiy:  一览页面到详情页面打开方式横展开,将返回按钮注释   2016/10/11   BY  zhukai1  end --%>
       </span>
</div>
<script src="${ctx}/static/sp/js/SP171117.js"></script>

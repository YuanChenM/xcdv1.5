 ...
 */
@Controller
@RequestMapping("SP171114")
public class SP171114Controller extends BaseController {

    /**
     * logger
     */
    private static Logger logger = LoggerFactory.getLogger(SP171114Controller.class);
    ...
    PageResult<SP171112Bean> search(BasePageParam param, @RequestParam(value = "pricePeriod", required = false) String pricePeriod, @RequestParam(value = "userType", required = false) String userType) {
        logger.debug("价盘查询");
        // 获得供应商的产品信息
        List<MskSellerServiceParam> pdTypeCodeList = new ArrayList<MskSellerServiceParam>();
        //调用pdTypeCode接口
        LoginUser loginUser = this.getLoginUser();
        MskSlInfoServiceParam slInfo = CommRestUtil.getSlInfo(loginUser.getEmplId());
        pdTypeCodeList = CommRestUtil.getPdResponse(slInfo);

...
(function($, undefined) {
		...
		return {
			...
			isAllpass: function(elements, options) {
					...
					, remind = function(control, type, tag) {
						...
						if ($(control).isVisible()) {
							if (type == "radio" || type == "checkbox") {
								...
								control.focus();
								control.select();
							} else if (tag == "exist") {//modify by yangchunyan
								var finalText = OBJREG.prompt[tag];
								$(control).testRemind(finalText);
								control.focus();
								control.select();
							}else {
								var finalText = '';
								if (tag == "exist") {//modify by yangchunyan
									finalText = OBJREG.prompt[tag];
								}else{
									// 文本框文本域格式不准确
									// 提示文字的获取
									finalText = OBJREG.prompt[type] || OBJREG.prompt["pattern"];
									if (text) {
				...
				elements.each(function(){
					var el = this, type = el.getAttribute("type"),uname=el.getAttribute("name"), tag = el.tagName.toLowerCase(), isRequired = $(this).hasProp("required");
					// type类型
					if (type) {
					...
					if (type == "radio" && isRequired) {
						...
						// 最大值最小值, 个数是否超出的验证
						allpass = false;
					}else if(type == "tel|xxxx"){
						var url = ''
						var data;
						if(uname == 'BA2141120') {
							BA2141120Flag = false;
							BA2141208Flag = true;
							url = ConstantDef.getFindSlAccountServerUrl();
							data = {
								param: {
									slAccount:{slTel:el.value}
								}
							};
							HttpClient.post(url,data,function(data){
								if(data.status == "F"){
									allpass = remind(el, type, "exist");
								}
								else{
									if(uname = 'BA2141120' && allpass && !BA2141120Flag){
										BA2141120Flag = allpass;
						...
						}else if(uname == 'BA2141208') {
							BA2141208Flag = false;
							BA2141120Flag = true;
							url = ConstantDef.getFindHouseTelServerUrl();
							data = {
								param: {
									houseTel:el.value
								}
							...
							HttpClient.post(url,data,function(data){
								if(data.status == "F"){
									...
									if(uname = 'BA2141208' && allpass && !BA2141208Flag){
										BA2141208Flag = allpass;
	...
	$.fn.extend({
		...
		testRemind: function(content, options) {
			...
			// 当前元素的位置，提示框的方向
			var offset = $(this).offset(), direction = "bottom";
			if (!offset) return $(this);
			var remindTop = offset.top - this.remind.outerHeight() - params.size  + 70;
